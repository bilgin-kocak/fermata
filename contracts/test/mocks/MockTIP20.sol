...
/// @notice Test stand-in for a Tempo TIP-20 precompile: exact ITIP20 signatures, 6 decimals,
/// EIP-2612 permit with domain (name(), "1", chainid, address), v must be 27/28, ecrecover only.
/// Emits Transfer then TransferWithMemo like the precompile; spending an allowance emits no Approval;
/// an unlimited (max) allowance is not decremented (precompile behaviour, tempo crates tip20/mod.rs).
contract MockTIP20 is ITIP20 {
    ...
    function _move(address from, address to, uint256 amount) internal {
        if (to == address(0) || bytes12(bytes20(to)) == 0x20c000000000000000000000) revert InvalidRecipient();
    ...
    function transferWithMemo(address to, uint256 amount, bytes32 memo) external virtual {
        _move(msg.sender, to, amount);
